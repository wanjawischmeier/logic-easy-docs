...
1. **The next state must exist.** Every transition must reference an existing state as its next state. A transition whose next-state pattern matches no existing state, or which points to a state that has been removed, is invalid (for example: `The transition from "q0" (input 0) has a next state that does not exist.`).
